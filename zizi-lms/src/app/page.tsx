...
import type { TopicSummary, KGData } from '@/src/types';

const TopicGalaxy = dynamic(() => import('@/src/components/TopicGalaxy'), { ssr: false });
...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetchKG().catch(() => ({ nodes: [], edges: [] } as KGData)),
    ]).then(([t, kg]) => {
      setTopics(t);
      setKgData(kg);
      setLoading(false);
  ...
  return (
    <main className="min-h-screen flex flex-col pt-14" style={{ background: '#0a0a0f' }}>
      {/* Ambient blobs */}
      ...
      {/* Hero */}
      <div className="relative z-10 flex flex-col items-center text-center pt-12 pb-10 px-6">
        <div className="inline-flex items-center gap-2 text-sm font-medium mb-6 rounded-full px-4 py-1.5"
          style={{ background: 'rgba(139,92,246,0.1)', color: '#a78bfa', border: '1px solid rgba(139,92,246,0.25)' }}>
          <span>✨</span> Adaptive AI Micro-Learning Platform
        </div>
        <h1 className="text-5xl sm:text-6xl font-extrabold mb-4 tracking-tight" style={{ color: '#f1f5f9' }}>
          Zizi{' '}
          <span style={{ background: 'linear-gradient(135deg, #8b5cf6 0%, #22d3ee 100%)',
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Byte
          </span>
        </h1>
        <p className="text-xl italic mb-3 font-light" style={{ color: '#94a3b8' }}>
        ...
        <p className="text-sm max-w-lg" style={{ color: '#64748b' }}>
          ...
          analogy-first micro-lessons you can grasp in 2 minutes — then build with code, then share.
        </p>
        ...
        {!loading && topics.length > 0 && (
          <div className="flex items-center gap-6 mt-6">
            <div className="text-center">
              <div className="text-2xl font-bold" style={{ color: '#8b5cf6' }}>{courseTopics.length}</div>
              <div className="text-xs" style={{ color: '#475569' }}>Course Modules</div>
            </div>
            <div className="w-px h-8" style={{ background: 'rgba(255,255,255,0.1)' }} />
            <div className="text-center">
              <div className="text-2xl font-bold" style={{ color: '#22d3ee' }}>{postTopics.length}</div>
              <div className="text-xs" style={{ color: '#475569' }}>Generated Posts</div>
            </div>
            <div className="w-px h-8" style={{ background: 'rgba(255,255,255,0.1)' }} />
            <div className="text-center">
              <div className="text-2xl font-bold" style={{ color: '#f1f5f9' }}>
                {topics.reduce((s, t) => s + (t.concepts?.length || 0), 0)}
              </div>
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6">
        {loading ? (
          <div className="w-full rounded-2xl animate-pulse"
            style={{ height: 600, background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(139,92,246,0.15)' }} />
        ) : kgData.nodes.length > 0 ? (
          <TopicGalaxy data={kgData} height={600} />
        ) : (
          <div className="w-full rounded-2xl flex flex-col items-center justify-center text-center gap-4 px-8"
            style={{ height: 480, background: 'rgba(139,92,246,0.03)', border: '1px solid rgba(139,92,246,0.15)' }}>
            <span className="text-5xl opacity-30">🌏</span>
            <p className="text-sm" style={{ color: '#64748b' }}>Knowledge graph is empty.</p>
        ...
        <p className="text-center text-xs mt-3" style={{ color: '#334155' }}>
          Click any topic node to begin · Scroll to zoom · Drag nodes to explore
        </p>
      </div>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {courseTopics.map((t) => (
              <Link key={t.id} href={`/learn/${t.id}`}
                ...
                style={{ border: '1px solid rgba(139,92,246,0.2)', background: 'rgba(139,92,246,0.07)', color: '#a78bfa' }}>
                {t.module_number && <span style={{ color: '#6b7280', fontFamily: 'monospace' }}>{t.module_number}</span>}
                {t.name}
              </Link>
            ))}
            {postTopics.map((t) => (
              <Link key={t.id} href={`/learn/${t.id}`}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all duration-200"
                style={{ border: '1px solid rgba(34,211,238,0.2)', background: 'rgba(34,211,238,0.06)', color: '#67e8f9' }}>
