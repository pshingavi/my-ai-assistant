...
const NODE_DETAILS: Record<string, NodeDetail> = {
  ...
  'lms-pipe': {
    title: 'Byte Generator', group: 'Pipeline', groupColor: '#6d28d9', icon: '📚',
    ...
    usage: 'GET /api/topic/{id}/concept/{c}/claude-interaction triggers this on cache miss. POST /p5sketch/regenerate clears and reruns.',
  ...
  'content-pipe': {
    title: 'LinkedIn Creator', group: 'Pipeline', groupColor: '#059669', icon: '🔄',
  ...
  sqlite: {
    ...
    why: 'Avoids re-generating expensive LLM outputs. 140 concepts pre-cached, served in <100ms. FastAPI is the only reader; Byte Generator is the only writer.',
...
const DIAGRAMS: Record<string, { nodes: Node<ServiceNodeData>[]; edges: Edge[] }> = {
  system: {
    nodes: [
      ...
      n('lms-pipe', 'Byte Generator', '📚', 'Pipeline', '#6d28d9', 0, 350, 'cache miss → generate'),
      n('chat-pipe', 'Chat Pipeline', '💬', 'Pipeline', '#4f46e5', 255, 350, 'KG+Dense→Cohere'),
      n('content-pipe', 'LinkedIn Creator', '🔄', 'Pipeline', '#059669', 540, 350, 'research→dedup→post'),
